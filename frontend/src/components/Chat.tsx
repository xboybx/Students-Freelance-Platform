...
export default function Chat({ bookingId, onClose }: ChatProps) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-96 bg-white shadow-xl flex flex-col z-50">
      {/* Header */}
      <div className="bg-gradient-to-r from-purple-600 to-pink-600 p-4 flex items-center justify-between text-white rounded-tl-2xl">
        <div>
          <h3 className="text-lg font-semibold">Chat</h3>
          {connectionStatus !== 'connected' && (
            <span className="text-xs opacity-75">Connecting...</span>
          )}
        </div>
        <button
          onClick={onClose}
      ...
      <form onSubmit={handleSendMessage} className="p-4 bg-white border-t border-purple-100">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-2 bg-purple-50 border-none rounded-full focus:ring-2 focus:ring-purple-500 placeholder-purple-400"
            disabled={connectionStatus !== 'connected'}
          />
          <button
            type="submit"
            disabled={connectionStatus !== 'connected' || !messageInput.trim()}
            className="p-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-full disabled:opacity-50 hover:opacity-90 transition-all duration-300 shadow-sm hover:shadow-md"
